...
const TradeForm: React.FC<TradeFormProps> = ({
  ...
  const calculatePnL = () => {
    ...
    if (!isClosing && !(trade && formData.exitPremium > 0) && !formData.spxClosePrice) {
      return 0;
    }
    ...
    if (formData.tradeType === "IRON_CONDOR" && formData.spxClosePrice > 0) {
      ...
      const isMaxProfit = formData.spxClosePrice > formData.sellPut && 
                          formData.spxClosePrice < formData.sellCall;
      
      if (isMaxProfit) {
        // Max profit: keep all premium (exit premium is 0.00)
        const maxProfit = formData.entryPremium * formData.contractQuantity * 100 - formData.fees;
        return maxProfit;
      } else {
        // Loss: 5-wide spread minus premium received
        // Standard iron condor spread width is 5 points ($500 per contract)
        const loss = (formData.entryPremium - 5.00) * formData.contractQuantity * 100 - formData.fees;
        return loss; // This will be negative
      }
    }
    
    // Standard calculation for other cases
    const grossPnL = (formData.entryPremium - formData.exitPremium) * formData.contractQuantity * 100;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.tradeDate || !formData.level || formData.entryPremium <= 0) {
      alert('Please fill in all required fields: Date, Level, and Entry Premium');
      return;
    }

    // Validate strike prices for Iron Condor
    if (formData.tradeType === "IRON_CONDOR") {
      if (formData.sellPut <= 0 || formData.buyPut <= 0 || formData.sellCall <= 0 || formData.buyCall <= 0) {
    ...
    const isMaxProfit = formData.tradeType === "IRON_CONDOR" && 
    ...
    if (formData.tradeType === "IRON_CONDOR" && formData.spxClosePrice > 0) {
      exitPremium = isMaxProfit ? 0.00 : 5.00;
    }

    // Determine if the trade should be closed
    const shouldClose = isClosing || 
    ...
    const tradeData: Partial<Trade> = {
      ...
      spxClosePrice: formData.spxClosePrice > 0 ? formData.spxClosePrice : undefined,
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    if (field === "spxClosePrice" && value > 0 && formData.tradeType === "IRON_CONDOR") {
      const isMaxProfit = value > formData.sellPut && value < formData.sellCall;
      
      // For iron condors with SPX close price, automatically set exit premium
      // If it's a win (SPX between sell strikes), exit premium is 0.00 (kept all premium)
      // If it's a loss (SPX outside sell strikes), exit premium is max loss (5.00)
      const exitPremium = isMaxProfit ? 0.00 : 5.00;
    ...
    <div className="trade-form-container">
      <div className="trade-form-header">
        ...
        <form onSubmit={handleSubmit} className="trade-form">
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleInputChange("level", e.target.value)}
                  disabled={isClosing}
                >
          ...
          {/* P&L Preview */}
          {(isClosing || formData.exitPremium > 0 || (formData.tradeType === "IRON_CONDOR" && formData.spxClosePrice > 0)) && (
                ...
                {formData.tradeType === "IRON_CONDOR" && formData.spxClosePrice > 0 ? (
                  ...
                  <>
                    ...
                    <div className="pnl-row">
                      ...
                      <span>${(formData.entryPremium * formData.contractQuantity * 100).toFixed(2)}</span>
                    </div>
                    {!(formData.spxClosePrice > formData.sellPut && formData.spxClosePrice < formData.sellCall) && (
                      <div className="pnl-row">
                        <span>Max Loss (5-wide spread):</span>
                        <span>-${(5.00 * formData.contractQuantity * 100).toFixed(2)}</span>
                      </div>
                    )}
                    <div className="pnl-row">
