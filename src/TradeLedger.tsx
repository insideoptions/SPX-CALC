...
const TradeLedger: React.FC<TradeLedgerProps> = ({ onTradeUpdate }) => {
  ...
  const [showCloseTradeModal, setShowCloseTradeModal] = useState<boolean>(false);
  ...
  const addTrade = async (newTrade: Partial<Trade>) => {
    ...
    try {
      ...
      const tradeWithLocalId: Trade = {
        ...
        matrix: newTrade.matrix || 'Standard Matrix',
        buyingPower: newTrade.buyingPower || '$26,350',
        spxClosePrice: newTrade.spxClosePrice,
        isMaxProfit: newTrade.isMaxProfit,
        seriesId: newTrade.seriesId
      };
      ...
      while (currentRetry < maxRetries && !awsSuccess) {
        try {
          ...
          // Create a complete trade object for AWS
          const completeTradeForAws: Omit<Trade, "id"> = {
            userId: user.id || user.uid || '',
            userEmail: user.email,
            tradeDate: newTrade.tradeDate || new Date().toISOString().split('T')[0],
            entryDate: newTrade.entryDate || new Date().toISOString(),
          ...
          awsTrade = await createTrade(completeTradeForAws);
  ...
  const updateTradeHandler = async (updatedTrade: Partial<Trade>) => {
    ...
    try {
      ...
      if (!updatedTrade.id) {
        console.error('Cannot update trade: No trade ID provided');
        return;
      }
      ...
      // Find the original trade to merge with updates
      const originalTrade = trades.find(t => t.id === updatedTrade.id);
      if (!originalTrade) {
        console.error(`Cannot update trade: Trade with ID ${updatedTrade.id} not found`);
        return;
      }
      ...
      while (currentRetry < maxRetries && !awsSuccess) {
        try {
          ...
          // Ensure we have a complete trade object for AWS
          const completeTradeForAws: Trade = {
            ...originalTrade,  // Use the original trade as base
            ...updatedTrade,   // Apply updates
            id: originalTrade.id, // Ensure ID is preserved
            userEmail: user.email,
            userId: user.id || user.uid || ''
          };
          
          awsTrade = await updateTrade(completeTradeForAws);
  ...
  // Handle closing a trade with TradeForm
  const handleCloseTrade = (trade: Trade) => {
    setTradeToClose(trade);
    setShowCloseTradeModal(true);
  };

  // Handle submitting the close trade form
  const handleCloseTradeSubmit = (updatedTrade: Partial<Trade>) => {
    if (tradeToClose && updatedTrade) {
      // Update the trade with the closed status and calculated P&L
      const closedTrade: Partial<Trade> = {
        ...updatedTrade,
        id: tradeToClose.id,
        status: "CLOSED",
      ...
      // Update the trade
      updateTradeHandler(closedTrade);
                ...
                <div className="series-trades">
                  {groupTrades.map((trade) => (
                    <div key={trade.id} className={`series-trade ${trade.status.toLowerCase()}`}>
                      ...
                      <div className="trade-actions">
                        <button className="action-btn" onClick={() => handleEditTrade(trade)}>
                          Edit
                        </button>
                        {trade.status === "OPEN" && (
                          <button className="action-btn close" onClick={() => handleCloseTrade(trade)}>
                            Close
                          </button>
                        )}
                        <button className="action-btn delete" onClick={() => deleteTradeHandler(trade.id)}>
                          Delete
                        </button>
                      </div>
                    </div>
